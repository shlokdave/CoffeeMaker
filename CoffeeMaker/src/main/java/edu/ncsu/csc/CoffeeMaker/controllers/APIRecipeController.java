...
public class APIRecipeController extends APIController {
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
        if ( recipe.getName().isEmpty() ) {
            return new ResponseEntity( errorResponse( "Recipe name cannot be blank" ), HttpStatus.BAD_REQUEST );
        }

        final HashSet<String> found = new HashSet<String>();
        for ( final var i : recipe.getIngredients() ) {
            if ( found.contains( i.getIngredient() ) ) {
                return new ResponseEntity( errorResponse( "Duplicate ingredient: " + i.getIngredient() ),
                        HttpStatus.BAD_REQUEST );
            }
            else if ( i.getIngredient() == null || i.getIngredient().isEmpty() ) {
                return new ResponseEntity( errorResponse( "Empty ingredient" ), HttpStatus.BAD_REQUEST );
            }
        }
